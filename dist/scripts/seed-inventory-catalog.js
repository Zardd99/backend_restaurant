...
async function run() {
    ...
    const supplier = await Supplier_1.Supplier.findOneAndUpdate({ name: "IMS Primary Distributor" }, {
        $setOnInsert: {
    ...
    const upsertIngredient = async (data, extra = { isPrepped: false }) => {
        var _a;
        const storage = (_a = data.storage) !== null && _a !== void 0 ? _a : STORAGE_BY_CATEGORY[data.category];
        const doc = await Supplier_1.Ingredient.findOneAndUpdate({ name: data.name }, {
            $set: {
    ...
    for (const prepped of PREPPED_INGREDIENTS) {
        ...
        preppedCount += 1;
        const components = prepped.recipe.map(([rawName, grossQuantity, netQuantity, unit]) => {
            const ingredientId = idByName.get(rawName);
            if (!ingredientId) {
                throw new Error(`Recipe for ${prepped.ingredient.name} references unknown raw ingredient "${rawName}"`);
            }
            return {
        ...
        const recipe = await Recipe_1.default.findOneAndUpdate({ targetId: preppedId, targetType: "Ingredient" }, { $set: { ingredients: components } }, { upsert: true, new: true, setDefaultsOnInsert: true });
        recipeCount += 1;
        await Supplier_1.Ingredient.updateOne({ _id: preppedId }, { $set: { recipeId: recipe._id } });
    }
    console.log(`Prepped ingredients upserted: ${preppedCount}`);
    console.log(`Recipes upserted: ${recipeCount}`);
    const total = await Supplier_1.Ingredient.countDocuments();
...
//# sourceMappingURL=seed-inventory-catalog.js.map